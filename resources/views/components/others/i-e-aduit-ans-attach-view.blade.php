...
@if ($attributes->has('shouldNotCollapse') && @$attributes['shouldNotCollapse'] === 'true')
  ...
  <div class="row">
    ...
    <div class="col-6 col-sm-6 eachImage">
      ...
      <div style="position: relative; display: inline-block;" id="table_data_delete" data-action="{{route('ie_audit_ans_file.destroy',$attachement->id)}}" data-parent="div.eachImage">
        ...
        <a test={{$attachement->id}} href="{{ asset("attachements/ie_audit/" . $attachement->file_name) }}">
            <img class="w-100 border-radius-lg p-1 shadow-lg" src="{{ asset("attachements/ie_audit/" . $attachement->file_name) }}" alt="attachements">
        </a>
    </div>
    @endforeach
</div>
