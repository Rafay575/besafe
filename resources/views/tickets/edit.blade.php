...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
      ...
      <li class="breadcrumb-item text-sm text-white active"><a class="opacity-5 text-white" href="javascript:;">Edit Ticket</a></li>
    </ol>
    <h6 class="font-weight-bolder mb-0 text-white">Edit Ticket</h6>
  </nav>
@endsection
              ...
              <div class="row">
                  ...
                  <select class="form-control" name="ticketsubtype_id" id="ticketsubtype_id" >
                      @foreach($ticketsubtypes as $ticketsubtype)
                        <option {{$ticket->ticketsubtype_id == $ticketsubtype->id ? "selected" : ""}} value="{{$ticketsubtype->id}}">{{$ticketsubtype->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  ...
                  <div class="form-group col-md-4">
                      <button type="submit" class="btn btn-primary w-50 mt-4 ">Update Ticket</button>
                  </div>
