...
<div class="row">
	...
	<div class="<?php echo e(auth()->user()->id == $ticket->assigned_to ? 'col-md-7' : 'col-md-9'); ?>  px-2 ">
		<?php if($ticket->feedback()->count() > 0): ?>
					...
					<div class=" p-4 bg-white-unique lightborder-unique h-100">
						...
						<div class=" d-flex justify-content-between align-items-center">
							<div class="d-flex align-items-center justify-content-between w-100">
								<div class="d-flex">
									<h6 class="mb-0"><strong><?php echo e($requested_byName->name); ?></strong> gave you a <?php echo e($rating); ?>-star
										review</h6>
									<i class="text-muted" style="margin-left:10px;"><?php echo e($created_at); ?></i>
								</div>
								<div>
						...
						<div class="card mt-4 lightborder-unique">
							<div class="mb-4">
								<div class="card-header lightborder-unique bg-light px-4 py-1">
									...
									<h6 class="my-2"><?php echo e($requested_byName->name); ?> REVIEW</h6>
								</div>
								<div class="d-flex align-items-start mb-2 card-body">
									<div>
									<?php if($requested_byName->image): ?>
											<img src="<?php echo e(asset('images/profile/' . $requested_byName->image)); ?>" height="70"
									...
									<div class="ms-4">
										...
										<strong><?php echo e($requested_byName->name); ?> message
											<span class="text-warning ms-2">
												<?php for($i = 1; $i <= $rating; $i++): ?>
													&#9733;
												<?php endfor; ?>
											</span>
											<strong class="ms-2"><?php echo e($rating); ?></strong></strong>

										<p style="font-size:12px;margin-top:5px"><?php echo e($review); ?></p>
									</div>
								</div>
							</div>
		...
		<div id="rafay-section-1" style="display:<?php echo e($ticket->feedback()->count() > 0 ? 'none' : 'block'); ?>">
			<div class="bg-white-unique pt-3 h-100 d-flex flex-column  lightborder-unique">
				...
				<div class="d-flex px-3 pb-3 justify-content-between align-items-center border-bottom ">
					...
					<?php if($ticket->status == "Open"): ?>
						<div class="d-flex">
							...
							<?php if(auth()->user()->id == $ticket->assigned_to): ?>

								<button type="button" class=" btn-unique btn-unique-outline-danger me-2" data-bs-toggle="modal"
									data-bs-target="#feedbackModal">
									Close</button>
								<a href="<?php echo e(route('tickets.reassign', $ticket->id)); ?>" class="mx-2" data-bs-toggle="tooltip"
				...
				<div class="card-body px-0 pt-0 overflow-auto flex-grow-1" style="height:60vh">
					<?php if($ticket->comments->count() > 0): ?>
						<?php $__currentLoopData = $ticket->comments->reverse(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<div class="  mb-5">
								<div class="">
									<div class="d-flex align-items-center border-bottom py-3 px-3 bg-light">
										<?php if($comment->commenter_user->image): ?>
											<img src="<?php echo e(asset('images/profile/' . $comment->commenter_user->image)); ?>" height="50"
										...
										<?php endif; ?>
										<div>
											<strong><?php echo e($comment->commenter_user->name); ?></strong>
											<small class="text-muted d-block">
												<?php echo e($comment->created_at->format('D, M d, Y h:i A')); ?>
									...
									<div class="comment-content mt-5 text-justify px-6"
										...
										<?php if($comment->comment_attachment): ?>
											<div class="btn-group" role="group">
												...
												<button type="button"
													...
													Document Attachment
													<!-- View icon -->
													<a href="<?php echo e(route('comment.attachment.view', $comment->id)); ?>" target="_blank"
														class="ms-2" title="View">
														<i class="fa fa-eye text-white" aria-hidden="true"></i>
													</a>
													<!-- Download icon -->
													<a href="<?php echo e(route('comment.attachment.download', $comment->id)); ?>" class="ms-2"
														title="Download">
										...
										<?php if($comment->voice_note_path): ?>
											<div class="mt-3">
												...
												<audio controls>
													<source
														src="<?php echo e(asset('comment_attachment/audios/' . $comment->voice_note_path)); ?>"
														type="audio/mpeg">
				...
				<?php if($ticket->status == "Open"): ?>
					<div class="border-top p-3 d-none w-100" id="commentForm">
						<form action="<?php echo e(route('comment.add')); ?>" method="POST" enctype="multipart/form-data" class="w-100">
							<?php echo csrf_field(); ?>
							<input type="hidden" name="ticket_id" value="<?php echo e($ticket->id); ?>">
							<input type="hidden" name="commenter" value="<?php echo e(auth()->user()->id); ?>">
	...
	<div class="col-md-3 px-2 ">
		<div class="bg-white-unique p-3  d-flex flex-column lightborder-unique">
			<h5 class="text-dark"><?php echo e($ticket->tickettype->name); ?></h5>
			<h6 class="text-dark" style="font-size:14px"><?php echo e($ticket->ticketsubtype->name); ?></h6>
			...
			<div class="form-group mb-3">
				...
				<div class="form-control-static border p-2 rounded" style="background-color: #f8f9fa;font-size:12px">
					<?php echo e($ticket->current_level); ?>
			...
			<div class="form-group mb-3">
				...
				<div class="form-control-static border p-2 rounded" style="background-color: #f8f9fa;font-size:12px">
					<?php echo e($assignedUserName->name); ?>
		...
		<div class="col-md-2 px-2">
			<div class="bg-white-unique p-3 d-flex flex-column lightborder-unique">
				...
				<div class="d-flex align-items-center flex-column  my-3 justify-content-center">
					...
					<?php if($requested_byName->image): ?>
						<div>
							<img src="<?php echo e(asset('images/profile/' . $requested_byName->image)); ?>" class="rounded-circle me-2"
					...
					<?php endif; ?>
					<h5 class="mb-0 mt-2" style="color: #000; font-weight: 700;font-size:14px"><?php echo e($requested_byName->name); ?>

					</h5>
					<small class="text-muted" style="font-size:12px"><?php echo e($designationName); ?></small>
					<div>
					</div>
					</>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="<?php echo e(route('ticket.close', $ticket->id)); ?>" method="post">
						<?php echo csrf_field(); ?>
						<input type="hidden" name="ticket_id" value="<?php echo e($ticket->id); ?>">
						<input type="hidden" name="commenter" value="<?php echo e(auth()->user()->id); ?>">
	...
	<?php if($ticket->status == "Closed" && !$ticket_feedback_exists && auth()->user()->id == $ticket->requested_by): ?>
		<div class="modal fade show" id="ratingModal" tabindex="-1" role="dialog" aria-labelledby="ratingModalLabel"
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="mx-auto"> <?php echo e($assignedUserName->name); ?> has closed this ticket</h5>
					</div>
					...
					<div class="modal-body">
							...
							<?php if($ticket->comments->isNotEmpty()): ?>
												...
												<div class="py-3 border-bottom">
													<div class="d-flex align-items-center border-bottom pb-3">
														...
														<div>
															<strong><?php echo e($lastComment->commenter_user->name); ?></strong>
															<small class="text-muted d-block">
																<?php echo e($lastComment->created_at->format('D, M d, Y h:i A')); ?>
													...
													<div class="comment-content mt-5 text-justify"
														style="white-space: normal; word-wrap: break-word;">
														<p><?php echo e($lastComment->description); ?></p>
													</div>
						...
						<form action="<?php echo e(route('feedback.add')); ?>" method="POST" enctype="multipart/form-data">
							<?php echo csrf_field(); ?>
							<input type="hidden" name="ticket_id" value="<?php echo e($ticket->id); ?>">
							<input type="hidden" name="reviewer" value="<?php echo e(auth()->user()->id); ?>">
							<div class="form-group">
	...
	<script src="<?php echo e(asset('assets/js/ticketview.js')); ?>"></script>
	<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\wamp64\www\flexi\resources\views/tickets/view.blade.php ENDPATH**/ ?>